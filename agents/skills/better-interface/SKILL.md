...
description: >-
  User-invoked, cross-discipline interface review that coordinates accessibility, better-layout, better-writing, better-typography, better-colors, and make-interfaces-feel-better. Use when explicitly invoked for a holistic review of a screen, flow, feature, or product interface. Supports quick and full review modes. Triggers on better-interface, full interface review, holistic UI audit, cross-discipline design review, review the whole interface.
...
This skill owns orchestration only. Accessibility rules belong to `accessibility`; structure to `better-layout`; copy to `better-writing`; type to `better-typography`; color to `better-colors`; visual polish to `make-interfaces-feel-better`. Never duplicate or override their rules here. For detailed motion work, that skill delegates to `apple-design`, `improve-animations`, and `review-animations`.
...
Identify the framework, styling system, component library, design tokens, supported viewports, and documented preview or test commands. Follow the project's established Tailwind, plain CSS, CSS-in-JS, token, and component conventions. Reconnaissance is read-only: do not install dependencies, access secrets, make network requests, start persistent services, or run commands that mutate source, fixtures, snapshots, databases, or generated assets without explicit user authorization.
...
Run only local, read-only checks whose behavior is known from project documentation or scripts already inspected. Do not treat a command named `test`, `lint`, or `preview` as inherently safe. Inspect an already available rendered interface when runtime behavior or visual judgment matters. Report the exact command or interaction and observed result. If a check cannot be run safely, label it **Not verified** and state what remains; never convert a verification gap into a finding.
...
- `Approve` — no actionable findings remain, every requested domain was reviewed, and the claimed coverage was verified.
