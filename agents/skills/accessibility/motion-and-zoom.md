...
For an existing codebase where opt-in is not immediately feasible, prefer targeted reduced-motion overrides on the known animated components. A global duration override is a temporary migration aid only: it can change application behavior when JavaScript relies on transition or animation timing, so use it only after auditing and testing those event-driven flows.
...
`0.01ms` rather than `none` may preserve `animationend`/`transitionend` events, but it does not prove event-driven behavior remains correct. Remove the global override after component-level reduced-motion variants are in place.
