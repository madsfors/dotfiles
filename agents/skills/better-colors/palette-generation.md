...
`findMaxChroma` is pseudocode, not a bundled implementation. Use a named color library or calculator that targets the required gamut. If no deterministic tool is available, describe the algorithm and mark generated values **Not verified** instead of inventing them.
...
The values below are illustrative. Recalculate them for the actual base color, hue, and target gamut.
...
When generating palettes for multiple hues, use the same **lightness** and **chroma percentage** as a starting grid. Neither equal L nor equal percentage of maximum chroma guarantees equal apparent brightness or vividness across hues; inspect and tune the rendered palette.
