...
Hit-area sizes and focus behavior are covered by the `accessibility` skill; visual polish (radius, shadows, animation) by the `make-interfaces-feel-better` skill; line length and text spacing by the `better-typography` skill.
